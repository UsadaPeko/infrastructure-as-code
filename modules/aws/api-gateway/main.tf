...
resource "aws_api_gateway_integration" "iac-api-gateway-integration-google-enter" {
  rest_api_id = "${aws_api_gateway_rest_api.iac-api-gateway.id}"
  resource_id = "${aws_api_gateway_resource.iac-api-gateway-resource-google-enter.id}"
...
# /google/exit
resource "aws_api_gateway_resource" "iac-api-gateway-resource-google-exit" {
  rest_api_id = "${aws_api_gateway_rest_api.iac-api-gateway.id}"
  parent_id   = "${aws_api_gateway_resource.iac-api-gateway-resource-google.id}"
  path_part   = "exit"
}
...
# /google/exit is GET
resource "aws_api_gateway_method" "iac-api-gateway-method-google-exit" {
  rest_api_id   = "${aws_api_gateway_rest_api.iac-api-gateway.id}"
  resource_id   = "${aws_api_gateway_resource.iac-api-gateway-resource-google-exit.id}"
  http_method   = "GET"
  authorization = "NONE"
}

# /google/exit is proxy into google.com
resource "aws_api_gateway_integration" "iac-api-gateway-integration-google-exit" {
  rest_api_id = "${aws_api_gateway_rest_api.iac-api-gateway.id}"
  resource_id = "${aws_api_gateway_resource.iac-api-gateway-resource-google-exit.id}"
  http_method = "${aws_api_gateway_method.iac-api-gateway-method-google-exit.http_method}"
  integration_http_method = "GET" # GET으로 Proxy
  type                    = "HTTP_PROXY"
  uri                     = "http://google.com"
}
